...
module.exports = {
  ...
  if (!player) {
    let embed = new MessageEmbed()
      .setColor("343434")
      .setDescription(" There is **__0__** songs in queue")
  ...
  if (songs === null) {
    songsForPageContent = `Add song type play or search `
  ...
  const msg = await message.channel.send(`\` **__NOW PLAYING__**: ${player.queue.current.title}\` \`\`\`css\n${songsForPageContent.join('\n')}\`\`\` `)
  await msg.react('⬆️')
  await msg.react('⬇️')
