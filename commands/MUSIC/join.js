...
run: async (client, message, args) => {
 ...
 let player = client.manager.players.get(message.guild.id);
		...
		if (!message.member.voice.channel.permissionsFor(message.guild.me).has('CONNECT')) {
			return embed("I need permission to **connect** your voice channel", message.channel).then(m => m.delete({ timeout: 10000 }));
		}
		...
		embed(" Successfully  connected to your  voice  channel <a:dark:866726600028061706>",message.channel)
			...
			try {
				...
				const embed = new MessageEmbed()
					.setColor("343434")
					.setDescription("<a:dark:866726600028061706> Successfully join your voice channel");
