...
if (!args[0]) {
			...
			const msg = await message.channel.send(`<a:dark:866726600028061706> Turning on **Television mode**.`);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`<a:Dot:873462741757755442> Turning off **television mode**.`);
			const embed = new MessageEmbed()
